...
def gradio_reset(chat_state, img_list):
    ...
    if img_list is not None:
        img_list = []
    return None, gr.update(value=None, interactive=True), gr.update(value=None, interactive=True), gr.update(placeholder='Please upload your video first', interactive=False),gr.update(value="Upload & Start", interactive=True), chat_state, img_list

# import glob
# video_folder = 'D:/research/video-1/1'  # 文件夹路径，根据实际情况进行修改
# video_files = glob.glob(video_folder + '*.mp4')  # 获取文件夹中所有的.mp4视频文件路径
# video_index = 0  # 当前读取的视频索引
...
#     else:
#         # 从文件夹中循环读取视频
#         video_path = video_files[video_index % len(video_files)]
#         video_index += 1

#         # 在这里处理视频文件的逻辑，可以调用相应的函数来处理视频
